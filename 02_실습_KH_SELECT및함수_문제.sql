...
select emp_name,
salary * 12, (salary + salary * nvl(bonus, 0)) * 12,
...
select emp_name,
salary, (salary + salary * nvl(bonus, 0) - salary * 0.03) * 12,
hire_date
    from employee
...
select emp_name, dept_code, hire_date
    from employee where dept_code in('D9', 'D5') and hire_date < to_date(20020101, 'yy/mm/dd')
;
-- 11. EMPLOYEE테이블에 고용일이 90/01/01 ~ 01/01/01인 사원의 전체 내용을 조회
select *
    from employee
    where hire_date between to_date(19900101, 'yy/mm/dd') and to_date(20010101, 'yy/mm/dd')
;
...
select *
    ...
    and hire_date between to_date(19900101, 'yy/mm/dd')
    and to_date(20010101, 'yy/mm/dd') and salary >= 2700000
;
...
select emp_name, floor(sysdate - hire_date) 근무일수1, floor(abs(hire_date - sysdate)) 근무일수2
    from employee
;
...
-- 주민번호 앞자리와 뒷자리의 합 조회
select emp_name,
substr(emp_no, 1, 6), substr(emp_no, 8, 14), substr(emp_no, 1, 6) + substr(emp_no, 8, 14)
    from employee
    where emp_id = 201
;
...
count(decode(substr(hire_date,1,2), '02', 2)) "2002년",
count(decode(substr(hire_date,1,2), '03', 3)) "2003년",
count(decode(substr(hire_date,1,2), '04', 4)) "2004년"
    from employee
;
select count(*) 전체직원수,
count(case when substr(hire_date,1,2) = '01' then 1 end) "2001년",
count(case when substr(hire_date,1,2) = '02' then 2 end) "2002년",
count(case when substr(hire_date,1,2) = '03' then 3 end) "2003년",
count(case when substr(hire_date,1,2) = '04' then 4 end) "2004년"
    from employee
;